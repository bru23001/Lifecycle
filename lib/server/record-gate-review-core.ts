...
export type RecordGateReviewInput = z.infer<typeof inputSchema>;

export type RecordGateReviewResult =
  | { ok: true; newPhase: number }
  | { ok: false; error: string; blockingMessages?: string[] };

export type RecordGateReviewCoreOptions = {
  /** When set, used for approval outcome and audit actor. */
  submittedById?: string | null;
  /** When false, skips audit row creation (not used by production server action). */
  writeAudit?: boolean;
};

/**
 * Records a gate review decision against the given Prisma client.
 * Shared by the server action and E2E harness (isolated DB).
 */
export async function recordGateReviewCore(
  db: PrismaClient,
  raw: RecordGateReviewInput,
  options: RecordGateReviewCoreOptions = {},
  ...
  const submittedById = options.submittedById ?? null;

  const gateDecisionId = await db.$transaction(async (tx) => {
