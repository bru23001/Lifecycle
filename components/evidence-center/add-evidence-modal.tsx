...
export function AddEvidenceModal({
  ...
  const [state, setState] = useState<AddEvidenceFormState>(() => emptyAddEvidenceFormState());
  ...
  function updateField<K extends keyof AddEvidenceFormState>(field: K, value: AddEvidenceFormState[K]) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (tab === "upload") {
      setError("File upload is not available yet. Use the Link tab to attach evidence via URL.");
      return;
    }
  ...
  return (
    ...
    >
      <form className="flex max-h-[85vh] flex-col" onSubmit={handleSubmit}>
        <header className="flex items-start justify-between gap-3 border-b border-slate-200 px-6 py-4 dark:border-border">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Upload coming soon — link external evidence and attach metadata for now.
            </p>
          </div>
          <button
            type="button"
        ...
        <div role="tablist" className="flex gap-1 border-b border-slate-200 px-6 dark:border-border">
          <TabButton
            active={tab === "upload"}
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-6 py-5 text-sm">
          {tab === "upload" ? (
            <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-600">
              <Upload className="mx-auto mb-2 size-6 text-slate-400" aria-hidden />
              <p className="font-medium text-slate-800">File upload is not available yet.</p>
              <p className="mt-1 text-xs">
                Switch to <strong>Add external link</strong> to attach evidence via URL with full metadata. A classified
                upload pipeline (STD-SEC-002 + STD-DAT-001) will follow in a future release.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Field id="evidenceType" label="Evidence type" required>
                  <select
                    ...
                    onChange={(e) => updateField("evidenceType", e.target.value as EvidenceFormType)}
                    className={inputClass}
                  >
                ...
                <Field id="classification" label="Classification" required>
                  <select
                    ...
                    onChange={(e) => updateField("classification", e.target.value as EvidenceClassification)}
                    className={inputClass}
                  >
              ...
              <Field
                id="sourceUrl"
                label={state.evidenceType === "link" ? "Source URL" : "Source URL (optional)"}
              ...
              <Field id="declaredFileName" label="Declared file name (optional)">
                <input
                  ...
                  maxLength={500}
                  onChange={(e) => updateField("declaredFileName", e.target.value)}
                  className={inputClass}
              ...
              <Field id="linkedArtifactId" label="Linked artifact (optional)">
                {artifactOptions && artifactOptions.length > 0 ? (
                  <select
                    ...
                    onChange={(e) => updateField("linkedArtifactId", e.target.value)}
                    className={inputClass}
                  >
...
function TabButton({
  ...
  return (
    ...
    >
      {icon}
      <span>{label}</span>
      {sublabel ? <span className="text-[10px] uppercase tracking-wide text-slate-400">· {sublabel}</span> : null}
    </button>
  );
}
