...
export function EvidencePreviewModal({
  ...
  onClose: () => void;
  /** `page` — inline surface for `/evidence/[id]/preview` (no modal chrome). */
  ...
  }, [open, selectedEvidence, presentation]);
  ...
  const href =
    selectedEvidence.detail.previewHref ?? selectedEvidence.detail.downloadHref;
  const downloadHref = selectedEvidence.detail.downloadHref;
  const sensitive = evidenceDownloadNeedsConfirmation(
    selectedEvidence.detail.classification,
  );
  ...
  const shellClass =
    presentation === "page"
      ? "mx-auto w-full max-w-[min(100vw-2rem,1200px)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-border dark:bg-card"
      : "fixed left-1/2 top-1/2 z-50 w-[min(100vw-1rem,min(96vw,1200px))] max-h-[92vh] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-slate-200 bg-white p-0 shadow-2xl backdrop:bg-slate-900/50 dark:border-border dark:bg-card";

  const innerMax =
    presentation === "page"
      ? "max-h-[min(100vh-10rem,92vh)] min-h-[320px]"
      : "max-h-[92vh]";

  const body = (
    <div className={cn("flex flex-col", innerMax)}>
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 px-5 py-4 dark:border-border">
        <div className="min-w-0">
          <h2
            id="evidence-preview-modal-title"
          ...
          >
            {selectedEvidence.detail.evidenceCode} ·{" "}
