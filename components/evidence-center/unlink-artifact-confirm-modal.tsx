...
import { unlinkEvidenceFromArtifact } from "@/app/actions/evidence";
...
export function UnlinkArtifactConfirmModal({
  open,
  projectId,
  evidenceId,
  evidenceSummary,
  ...
  const submit = () => {
    const r = reason.trim();
    if (r.length < 1) {
      reportError("A reason is required to unlink this artifact.");
      return;
    }
    startTransition(async () => {
      const res = await unlinkEvidenceFromArtifact({
        projectId,
        evidenceId,
        artifactId,
        reason: r,
  ...
  return (
    <dialog
      ...
      aria-labelledby="unlink-artifact-title"
    >
      <div className="flex max-h-[85vh] flex-col">
        <header className="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-4 dark:border-border">
          <div>
            <h2 id="unlink-artifact-title" className="text-lg font-semibold text-slate-900 dark:text-foreground">
              Unlink artifact?
            </h2>
            <p className="mt-2 text-sm text-slate-600">
              This removes the association between evidence <span className="font-medium text-slate-800">{evidenceSummary}</span> and
              artifact <span className="font-medium text-slate-800">{artifactLabel}</span>. The evidence file and the artifact record are
              not deleted.
            </p>
          </div>
        ...
        <div className="space-y-3 overflow-y-auto px-5 py-4 text-sm">
          ...
          <label className="block">
            ...
            <textarea
              ...
              placeholder="Explain why this link should be removed (audit trail)."
              maxLength={2000}
            />
          </label>
        </div>
