...
function StatusBadge({ gate }: { gate: DashboardGateStatus }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className={["text-sm font-semibold", statusClasses[gate.label]].join(" ")}>{gate.count}</span>

      <span
        className={[
          "flex h-5 w-5 items-center justify-center rounded-full border-2 text-[10px] font-semibold",
      ...
      <span className={["text-sm font-semibold", statusClasses[gate.label]].join(" ")}>{gate.label}</span>
    </div>
  );
}
...
const rowGridClass =
  "grid grid-cols-1 gap-2 xl:grid-cols-[24px_minmax(140px,1.2fr)_minmax(130px,0.9fr)_minmax(72px,84px)] xl:items-center";
      ...
      <header className="mb-4 flex items-center justify-between gap-2">
        <div className="min-w-0">
          {defaultReviewHref ? (
            ...
            <h2 className="text-lg font-semibold text-slate-950 dark:text-slate-100">Gate Status Summary</h2>
          )}
        </div>
        <Link
        ...
        {gateStatuses.map((gate) => {
          ...
          const rowInner = (
            <>
              ...
              <div className="w-full max-w-[84px] xl:justify-self-end">
          ...
          return rowHref ? (
            <Link
              ...
              className={cn(
                rowGridClass,
                "-mx-2 rounded-lg px-2 py-0.5 outline-none ring-offset-white transition hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-[#2563eb] dark:ring-offset-card dark:hover:bg-slate-800/40",
              )}
            >
              {rowInner}
            </Link>
